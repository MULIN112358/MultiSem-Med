...
class MultiSemMed(nn.Module):
    def __init__(
        ...
        if args.model_name == 'rnn':
            self.encoders = nn.ModuleList(
                [nn.RNN(input_size=emb_dim, hidden_size=emb_dim, batch_first=True)
                 for _ in
                 range(K - 1)]
            )
        if args.model_name == 'lstm':
            self.encoders = nn.ModuleList(
                [nn.LSTM(input_size=emb_dim, hidden_size=emb_dim, batch_first=True)
                 for _ in
                 range(K - 1)]
            )
        if args.model_name == 'gru':
            self.encoders = nn.ModuleList(
                [nn.GRU(input_size=emb_dim, hidden_size=emb_dim, batch_first=True)
                 for _ in
                 range(K - 1)]
            )
        ...
        self.w_bce = nn.Parameter(torch.tensor(1.0))
        self.w_multi = nn.Parameter(torch.tensor(1.0))
    ...
    def forward(self, input):
        ...
        if args.model_name == 'rnn' or args.model_name == 'lstm' or args.model_name == 'gru':
            o1, _ = self.encoders[0](i1_seq)  # o1:(1, seq, dim)
